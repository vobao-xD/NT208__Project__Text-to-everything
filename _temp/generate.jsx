...
const handleFileUpload = async (file) => {
	...
	try {
		...
		if (analyzeResult.success) {
			...
			setChatHistory((prev) =>
				prev.filter(
					(m) => !(m.type === "user" && m.content === extractedText)
				)
			);
		} else {
			toast.error(
				...
				>
					{message.type === "user" ? (
						...
						<a
							...
							download={message.content.file_name || "file"}
							target="_blank"
							rel="noopener noreferrer"
							className="file-link"
							style={{
						...
						>
							<span style={{ fontSize: "24px" }}>📄</span>
							<span>
								{message.content.file_name || "Tệp đính kèm"}
							</span>
						</a>
					) : message.isText ? (
						<p className="text-message">{message.content.text}</p>
					) : null}
				</div>
			))}
						...
						{localStorage.getItem("role") !== "free" ? (
							<>
								...
								<span style={{ color: "#fff" }}>
									Hãy chọn file (.wav,.mp3) để chuyển thành
									văn bản...
								</span>
							</>
						) : (
							...
							>
								...
								>
									...
									Vui lòng nâng cấp lên Plus hoặc Pro để sử
									dụng tính năng này!
								</span>
					...
					>
						{role !== "free" ? (
							<>
								...
								/>
								<span style={{ color: "#fff" }}>
									Hãy chọn file video (.mp4) để chuyển thành
									văn bản...
								</span>
							</>
						) : (
					...
					>
						{role !== "free" ? (
							<>
								...
								/>
								<span style={{ color: "#fff" }}>
									Hãy chọn file (.pdf, .doc, .docx, .txt) để
					...
					>
						{role !== "free" ? (
							<>
								...
								/>
								<span style={{ color: "#fff" }}>
									Hãy chọn file ảnh (.jpg) để cải thiện chất
									lượng...
								</span>
							</>
						) : (
