...
from . import authentication, text_to_speech, text_to_image, text_to_video, general, history,MOMO,input,ChatBotContent,enhance,Check_api,generate_answer,text_to_code,role,advanced_model

router = APIRouter()

router.include_router(general.router, prefix="", tags=["General"])
router.include_router(authentication.router, prefix="", tags=["Auth"])
