	...
	const getPricing = () => {
		if (activeTab === "monthly") {
			...
			return {
				plus: {
					amount: 1000000,
					display: "1.000.000 VND/Năm",
					original: "1.200.000 VND",
					discount: "Tiết kiệm 200.000 VND",
				},
				pro: {
					amount: 2000000,
					display: "2.000.000 VND/Năm",
					original: "2.400.000 VND",
					discount: "Tiết kiệm 400.000 VND",
				},
			};
		}
	};
		...
		if (plan === "plus") {
			...
			if (currentRole === "pro" && currentBillingCycle === activeTab) {
				...
				toast.warning("Bạn đã sử dụng gói Pro với chu kỳ này", {
					closeButton: true,
					className:
						"p-0 w-[400px] border border-yellow-600/40 backdrop-blur-lg",
					ariaLabel: "Warn",
				});
				return;
			}
		...
		try {
			...
			if (response.ok) {
				...
				toast.info("Thanh toán thành công!", {
					closeButton: true,
					className:
						"p-0 w-[400px] border border-green-600/40 backdrop-blur-lg",
					ariaLabel: "Info",
				});
			...
			>
				<button
					...
					style={{
						...
						border: "none",
						background:
							activeTab === "monthly" ? "#007bff" : "transparent",
				...
				<button
					...
					style={{
						...
						border: "none",
						background:
							activeTab === "yearly" ? "#007bff" : "transparent",
			...
			<div className="container">
				<div className="plan free">
					...
					<ul>
						...
						<li>
							✅ Text-to-Speech (giọng cơ bản, giới hạn thời
							gian/ngày)
						</li>
						<li>
							✅ Speech-to-Text cơ bản (hỗ trợ tiếng Việt và tiếng
							Anh)
						</li>
					...
					<button
						id="free-button"
						className={`btn ${
							getButtonStatus("free").disabled
								? "disabled-btn"
								: ""
						}`}
						style={{
							marginTop: "100px",
							cursor: getButtonStatus("free").disabled
								? "not-allowed"
								: "pointer",
							backgroundColor: getButtonStatus("free").disabled
								? "white"
								: "",
							color: getButtonStatus("free").disabled
								? "black"
								: "",
						}}
				...
				<div className="plan plus">
					...
					<ul>
						...
						<li>
							✅ Text-to-Image nâng cao (AI chất lượng cao, giảm
							thời gian chờ)
						</li>
						<li>
							✅ Text-to-Speech (giọng AI tự nhiên, nhiều ngôn
							ngữ)
						</li>
						<li>
							✅ Speech-to-Text nâng cao (chính xác hơn, hỗ trợ đa
							giọng)
						</li>
					...
					<button
						...
						className={`btn upgrade ${
							getButtonStatus("plus").disabled
								? "disabled-btn"
								: ""
						}`}
						style={{
							marginTop: activeTab === "yearly" ? "20px" : "48px",
							cursor: getButtonStatus("plus").disabled
								? "not-allowed"
								: "pointer",
							backgroundColor: getButtonStatus("plus").disabled
								? "white"
								: getButtonStatus("plus").isUpgrade
								? "#28a745"
								: "",
							color: getButtonStatus("plus").disabled
								? "black"
								: "",
						}}
				...
				<div className="plan pro">
					...
					<p>
						Truy cập nhanh hơn, lưu trữ lớn hơn, quyền sử dụng cao
						nhất
					</p>
					<ul>
						<li>🔥 Tất cả quyền lợi của gói Plus</li>
						<li>
							✅ Text-to-Video (AI tạo video từ văn bản, nhân vật
							ảo)
						</li>
						<li>
							✅ AI Character Animation (tạo nhân vật động theo
							giọng nói)
						</li>
					...
					<button
						id="pro-button"
						className={`btn upgrade ${
							getButtonStatus("pro").disabled
								? "disabled-btn"
								: ""
						}`}
						style={{
							marginTop: activeTab === "yearly" ? "63px" : "91px",
							cursor: getButtonStatus("pro").disabled
								? "not-allowed"
								: "pointer",
							backgroundColor: getButtonStatus("pro").disabled
								? "white"
								: getButtonStatus("pro").isUpgrade
								? "#28a745"
								: "",
							color: getButtonStatus("pro").disabled
								? "black"
								: "",
						}}
